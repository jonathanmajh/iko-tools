...
def compare_values(target, results, ws, ws_write_col):
    ...
    if (results[15][-1] != '-'):
        if ((target <= results[15][-1])):
            ws.cell(row=3 * site_i + 1, column=ws_write_col).fill = green
        elif ((target - results[15][-1]) < (0.2 * target)):
    ...
    if (results[12][-1] != '-'):
        if ((target <= results[12][-1])):
            ws.cell(row=3 * site_i + 2, column=ws_write_col).fill = green
        elif ((target - results[12][-1]) < (0.2 * target)):
        ...
        averages[ENDPOINT_URL[ws_write_col-8]][0] += results[12][-1] 
        averages[ENDPOINT_URL[ws_write_col-8]][1] += 1
    if ((results[12][-1] == '-') or (results[15][-1] == '-')):  # blank is meh
        ws.cell(row=3 * site_i + 3, column=ws_write_col, value=0)
        overall['side'] += 1
    elif (abs(results[15][-1] - results[12][-1]) < 0.005):  # very small change is meh
        ws.cell(row=3 * site_i + 3, column=ws_write_col, value=0)
        overall['side'] += 1
    elif (results[15][-1] < results[12][-1]):  # above average
...
green = PatternFill(start_color='00C6EFCE',
                    end_color='00C6EFCE', fill_type='solid')
yellow = PatternFill(start_color='00FFEB9C',
                     end_color='00FFEB9C', fill_type='solid')
red = PatternFill(start_color='00FFC7CE',
                  end_color='00FFC7CE', fill_type='solid')
...
ws.append([
    '', '', '', '', '', 'Target','',0.95,0.8,0.5,0.8,0.8,'-',0.8,0.1,0.8,0.9,25,
])
...
ws.cell(row=4,column=2).border = Border(top=double, left=double, right=double, bottom=double)
...
ws.cell(row=10,column=3).border = Border(top=double, left=double, right=double, bottom=double)
...
ws.cell(row=28,column=4).border = Border(top=double, left=double, right=double, bottom=double)
...
ws.cell(row=40,column=5).border = Border(top=double, left=double, right=double, bottom=double)
...
ws.cell(row=2,column=18).border = Border(right=double,bottom=double, top=double)
ws.cell(row=3,column=18).border = Border(right=double,bottom=double, top=double)
...
for site in SITES:
    ...
    ws.cell(row=3 * site_i + 1, column=18).border = Border(right=double, top=double)
    ...
    if (result == -1):
        ...
        for key in allcount.keys():
            results[12 - (current_month - key)
                    ].append(ontimecount[key] / allcount[key])
            overall_percent += ontimecount[key] / allcount[key]
        if (len(allcount.keys()) > 0):
            results[15].append(overall_percent/len(allcount.keys()))
        else:
            ...
            if (result != -1):
                for item in result['info']:
                    monthdate = item['year'] * 12 + item['month']
                    results[12 - (current_month - monthdate)
                            ].append(item['percentage'])
                    overall_percent += item['percentage']
                if (len(result['info']) > 0 and overall_percent > 0.001):
                    results[15].append(overall_percent/len(result['info']))
    results = level(results)
    ws.cell(row=3 * site_i + 1, column=ws_write_col, value=results[15][-1])
    ws.cell(row=3 * site_i + 2, column=ws_write_col, value=results[12][-1])
    ...
    result = request_wrapper(
        f'{URL[0]}{ENDPOINT_URL[1]}{URL[1]}{site}{URL[2]}')
    if (result == -1):
        for j in range(len(results) - 2):
            results[j + 2].append("-")
    else:
    ...
    result = request_wrapper(
        f'{URL[0]}{ENDPOINT_URL[2]}{URL[1]}{site}{URL[2]}')
    if (result == -1):
        for j in range(len(results) - 2):
            results[j + 2].append("-")
    else:
    ...
    result = request_wrapper(
        f'{URL[0]}{ENDPOINT_URL[3]}{URL[1]}{site}{URL[2]}')
    if (result == -1):
        for j in range(len(results) - 2):
            results[j + 2].append("-")
    else:
    ...
    result = request_wrapper(
        f'{URL[0]}{ENDPOINT_URL[4]}{URL[1]}{site}{URL[2]}')
    if (result == -1):
        for j in range(len(results) - 2):
            results[j + 2].append("-")
    else:
    ...
    result = request_wrapper(
        f'{URL[0]}{ENDPOINT_URL[6]}{URL[1]}{site}{URL[2]}')
    if (result == -1):
        for j in range(len(results) - 2):
            results[j + 2].append("-")
    else:
    ...
    result = request_wrapper(
        f'{URL[0]}{ENDPOINT_URL[7]}{URL[1]}{site}{URL[2]}')
    if (result == -1):
        for j in range(len(results) - 2):
            results[j + 2].append("-")
    else:
    ...
    result = request_wrapper(
        f'{URL[0]}{ENDPOINT_URL[8]}{URL[1]}{site}{URL[2]}')
    if (result == -1):
        for j in range(len(results) - 2):
            results[j + 2].append("-")
    else:
    ...
    result = request_wrapper(
        f'{URL[0]}{ENDPOINT_URL[9]}{URL[1]}{site}{URL[2]}')
    if (result == -1):
        for j in range(len(results) - 2):
            results[j + 2].append("-")
    else:
    ...
    if (result == -1):
        ...
        for item in result['info']:
            monthdate = item['year'] * 12 + item['month']
            results[12 - (current_month - monthdate)].append(item['total'])
...
for average in averages:
    ws.cell(row=70,column=8+i,value=averages[average][0]/averages[average][1])
    i += 1
...
wb.save(
    filename=f"C:\\Users\\majona\\GitHub\\iko-tools\\Python\\KPI\\{date.today().isoformat()}_KPI.xlsx")

...
def compare_values(target, results, ws, ws_write_col):
    ...
    if (results[5][-1] == "-") or (results[3][-1] == "-"):  # blank is meh
        ...
        overall["side"] += 1
    elif abs(results[3][-1] - results[5][-1]) < 0.005:  # very small change is meh
        ws.cell(row=3 * site_i + 3, column=ws_write_col, value=0)
        overall["side"] += 1
...
ws.cell(row=2, column=11).fill = PatternFill(
    start_color="FABF8F", end_color="FABF8F", fill_type="solid"
)
ws.cell(row=2, column=12).fill = PatternFill(
...
ws.cell(row=4, column=20).fill = PatternFill(
    start_color="00B0F0", end_color="00B0F0", fill_type="solid"
)
ws.cell(row=5, column=21, value="Inventory Coordinator")
ws.cell(row=5, column=20).fill = PatternFill(
    start_color="B1A0C7", end_color="B1A0C7", fill_type="solid"
)
ws.cell(row=6, column=21, value="Reliability Engineer")
ws.cell(row=6, column=20).fill = PatternFill(
    start_color="FABF8F", end_color="FABF8F", fill_type="solid"
)
ws.cell(row=7, column=21, value="Maintenance Manager")
ws.cell(row=7, column=20).fill = PatternFill(
    start_color="FF6565", end_color="FF6565", fill_type="solid"
)
...
ws.cell(row=37, column=5).border = Border(
    top=double, left=double, right=double, bottom=double
)
...
for site in SITES:
    ...
    if result == -1:
        ...
        for key in allcount.keys():
            results[5 + (current_month - key)].append(ontimecount[key] / allcount[key])
        ...
        if len(allcount.keys()) > 0:
            ...
            result = request_wrapper(f"{URL[0]}{PMONTIME}{URL[1]}{site}{URL[2]}")
            if result != -1:
                for item in result["info"]:
                    monthdate = item["year"] * 12 + item["month"]
                    results[5 + (current_month - monthdate)].append(item["percentage"])
    ...
    generic_result_reader(
        f"{URL[0]}IKO_KPI_MAXIMOVSKRONOSHOURS{URL[1]}{site}{URL[2]}",
        results,
        ws,
        ws_write_col,
    )
    ...
    if result == -1:
        ...
        for monthdate in allcount.keys():
            results[5 + (current_month - monthdate)].append(allcount[monthdate])
        if len(result["info"]) > 0:
            results[3].append(overall_percent / len(allcount.keys()))
    results = level(results)
    ws.cell(row=3 * site_i + 1, column=ws_write_col, value=results[3][-1])
    ...
    generic_result_reader(
        f"{URL[0]}IKO_KPI_MROLINEFROMMAXIMO{URL[1]}{site}{URL[2]}",
        results,
        ws,
        ws_write_col,
    )
    ...
    generic_result_reader(
        f"{URL[0]}IKO_KPI_CYCLECOUNT{URL[1]}{site}{URL[2]}", results, ws, ws_write_col
    )
    ...
    generic_result_reader(
        f"{URL[0]}IKO_KPI_SCHEDULEDWORKORDERS{URL[1]}{site}{URL[2]}",
        results,
        ws,
        ws_write_col,
    )
    ...
    result = request_wrapper(f"{URL[0]}IKO_KPI_JOBPLANCREATED{URL[1]}{site}{URL[2]}")
    if result == -1:
        for j in range(len(results) - 2):
            results[j + 2].append("-")
    else:
